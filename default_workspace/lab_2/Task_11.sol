...
contract Task_11 {
    ...
    // 1) общий баланс (сумма всех депозитов минус возвраты)
    ...
    function withdraw() external whenActiveOrPaused {
        ...
        // interaction
        (bool ok, ) = payable(msg.sender).call{value: userBalance}("");
        require(ok, "Transfer failed");

        emit Withdrawn(msg.sender, userBalance);
    }

    // В закрытом состоянии владелец забирает всё, депозит/возврат недоступны
